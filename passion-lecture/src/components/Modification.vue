...
import { useRoute, useRouter } from 'vue-router' // importe le routeur
import { getBookById, updateBook } from '@/services/bookServices'

const route = useRoute()
const router = useRouter()
const bookId = route.params.id // récupère l'id de l'url ex: /modification/1
...
// charger les infos du livre séléctionné pour la modif au montage du composant
onMounted(async () => {
  if (bookId) {
    try {
      const response = await getBookById(bookId)
      newBook.value = response.data
    } catch (error) {
      // si erreur
...
// modifie les info sur la db.json a la place d'ajouter
const submitForm = async () => {
  try {
    await updateBook(bookId, newBook.value)
    alert('Ouvrage mis à jour avec succès !')
    router.push(`/Catalogue`) // Redirige vers la page catalogue après la modification
  } catch (error) {
    //si erreur
